...
This dataset contains information about the payment methods that the restaurants accept. It contains the following features:
...
title <- "Total restaurants per payment type"
xlab <- "RPayment"
...
##### Feature 2.1: Rcuisine

A graphical visualization with a barplot shows the distribution of this feature.
```{r}
rcuisine <- unique(chefmozcuisine_df$Rcuisine)
title <- "Total restaurants per cuisine type"
xlab <- "Rcuisine"
ylab <- "Restaurant count"
feature <- "Rcuisine"
drawBarplot(chefmozcuisine_df, rcuisine, title, xlab, ylab, feature)
```
...
From this analysis we conclude that this dataset has no missing values, but a treatment to parking_lot is necessary to reduce the number of levels of this feature.
```{r}
# Columns
