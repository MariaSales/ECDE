...
```{r}
user_ratings <- merge(rating_final_df, userprofile_df, by = "userID")

mean_low_rating <- mean(user_ratings[which(user_ratings$budget == "low"),]$rating)
mean_medium_rating <- mean(user_ratings[which(user_ratings$budget == "medium"),]$rating)
mean_high_rating <- mean(user_ratings[which(user_ratings$budget == "high"),]$rating)

mean_low_service <- mean(user_ratings[which(user_ratings$budget == "low"),]$service)
mean_medium_service <- mean(user_ratings[which(user_ratings$budget == "medium"),]$service)
mean_high_service <- mean(user_ratings[which(user_ratings$budget == "high"),]$service)

mean_low_food <- mean(user_ratings[which(user_ratings$budget == "low"),]$food)
mean_medium_food <- mean(user_ratings[which(user_ratings$budget == "medium"),]$food)
mean_high_food <- mean(user_ratings[which(user_ratings$budget == "high"),]$food)
...
# Make a basic graph
plot(rating~budget, type="b", main="Mean rating per budget type", bty="l", xlab="Budget type", ylab="Mean rating", col=rgb(0.2,0.4,0.1,0.7), lwd=3, pch=17 ,ylim=c(1, 2))
...
# Add a legend
legend("topright", legend = c("Combined", "Service", "Food"), col = c(rgb(0.2,0.4,0.1,0.7), rgb(0.8,0.4,0.1,0.7), rgb(0.6,0.3,0.2,0.7)), pch = c(17,18,19), bty = "n", pt.cex = 2, cex = 1.2, text.col = "black", horiz = F , inset = c(0.1, 0.1))
...
As observed in Data Understanding, this dataset has an attribute (Rcuisine) which has 59 levels. An important treatment to this feature is to reduce the amount of levels. Rcuisine represents the type of the cuisine of the restaurant, so the following levels will be created and used (instead of the originals presented in the dataset):
1. Bar/Cafeteria 
  - Bagels, Bakery, Bar, Bar_Pub_Brewery, Breakfast-Brunch, Cafe-Coffee_Shop, Cafeteria, Deli-Sandwiches, Dessert-Ice_Cream, Juice      
2. Mexican
  - Mexican, Regional
3. Asian
  - Asian, Chinese, Japanese, Korean, Mongolian, Sushi, Thai, Vietnamese
...
restaurant_cuisine_df <- chefmozcuisine_df
restaurant_cuisine_df$cuisine <- c("")

# Compute new cuisine types
for(i in seq_len(nrow(restaurant_cuisine_df))) {
  cuisine <- as.character(restaurant_cuisine_df[i,2])
  if(cuisine == "Bagels" || cuisine == "Bakery" || cuisine == "Bar" || cuisine == "Bar_Pub_Brewery" || cuisine == "Breakfast-Brunch" || cuisine == "Cafe-Coffee_Shop" || cuisine == "Cafeteria" || cuisine == "Deli-Sandwiches" || cuisine == "Dessert-Ice_Cream" || cuisine == "Juice") {
    restaurant_cuisine_df[i,3] <- "Bar/Cafeteria"
  } else if(cuisine == "Mexican" || cuisine == "Regional") {
    restaurant_cuisine_df[i,3] <- "Mexican"
  } else if(cuisine == "Asian" || cuisine == "Chinese" || cuisine == "Japanese" || cuisine == "Korean" || cuisine == "Mongolian" || cuisine == "Sushi" || cuisine == "Thai" || cuisine == "Vietnamese") {
    restaurant_cuisine_df[i,3] <- "Asian"
  } else {
    restaurant_cuisine_df[i,3] <- "International"
  }
}

# Transform Rcuisine to calculated values as factor
restaurant_cuisine_df$Rcuisine <- as.factor(restaurant_cuisine_df$cuisine)
# Drop unnecessary column
restaurant_cuisine_df <- restaurant_cuisine_df[,c("placeID", "Rcuisine")]
